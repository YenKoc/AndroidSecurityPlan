...
# 2021年7月10日
1. 摸鱼了一上午，下午和朋友吃饭了
2. 晚上学了点advmp的东西，发现一些本质，advmp本质还是在模仿art解释器，通过分析源码，
不同安卓版本是有不同解释类型的，swich case，goto表，核心还是这两种，汇编那种就开发而言，
还是太难，不过switch最后也是被优化成类似goto表了（，类似invoke指令变成用jni接口实现
，很大程度是直接实现太难，而且还需要考虑兼容性。
# 2021年7月11日
1. 对fart深度跟着玩玩，发现自己的几个问题，第一个就是编译源码刷机不熟练，尤其是部分编译，再刷入，
这样对沙箱定制会比较快，毕竟从头编译一个镜像太慢了
2. frida实现fart还是遇到了不少的问题，还在研究中
# 2021年7月12日
1. frida实现fart基本实现，怎么说了，之前一直不理解的东西，比如java层的inovke，以及在jni中的主动链的问题，突然就理解了，果然时间是个好东西，hh
2. 继续working，搞了一些东西，希望产出啊
3. 手动实现了一套art解释器，advmp其实就是一套jni自己实现的解释器，慢慢搞着，之后修复一手360加固
# 2021年7月13日
1. working了一整天，看了好多文章（具体保密），感觉有点思路了，打算继续日日
# 2021年7月14日
1. 干了一天vmp属于是脑子发热了，好累（，网上这种资料还是很少，只能自己慢慢摸索了，唉，没啥办法
2. 学了一招源码过360反调试的骚操作，太猛了，周末把360给干了先，把环境先搭好，编译起来

# 2021年7月15日
1. 逐渐摸清了vmp的结构，主要实现在于不是使用常见的switch case方式进行解释跳转，而是自己实现了跳板函数，
先取出opcode偏移，取出opcode，再进行跳转，不过是基于栈还是寄存器还待观察，感觉和常规的很大不同。
2. 把编译的环境重新配置了一下。

# 2021年7月16日
1. 想着把之前指纹哥的文章复现复现，复现了一部分，没办法太晚了，要回家了
2. 嫖上了ida7.6，太爽了
3. 环境真恶心啊，太大了，这严重拖慢我进度了，草
# 2021年7月17日
1. 感觉弄环境配置真恶心，尤其是国内目前创作环境（就是cxdn），各种抄袭的文章
点进去好几个都一摸一样，但是并不是我想要的那种，各种百度谷歌依旧没找到问题，挺郁闷的
，毕竟这玩意花费的时间实在太多了，今天暂时先这样把

# 2021年7月18日
1. 和教练，kp，2st，ssj一起恰了个饭，还蛮开心
2. 被环境干倒的一天，明天继续
# 2021年7月19日
1. 基本上把tx二代抽取壳手脱完成，说了几个有趣的点，一个是
将源dex藏在壳dex的linkoff处，然后再解压，通过mmap加载到内存中，
一个是hook了系统函数，本身构造了一个空文件，但是每次操作都会被hook到
之前解压出来加载到内存中的地址，还有一个，可以动态注册libc中的jni函数，有点骚，也就是相当于直接inlinehook了，笑死，这尼玛谁想的，太损了2333，
2. 遇到一个ida调试的痛点，打算写写插件解决一手
# 2021年7月20日
1. 看了很久的idapython的文档，有点迷，妈的
也没个example，只能自己慢慢摸了
# 2021年7月21日
1. 基本上把插件魔改完，实现了我自己想要的效果，hh。
# 2021年7月22日
1. 把代码优化了下，已经开源了
2. 继续开始working了，遇到了点小问题，不过问题不大，研究一手