...
# 2020年1月4日
1. 复盘一下去哪儿网的面试（反思一下自己的不足）
- 自我介绍
- 比赛经历和在校经历
- java和c++用的熟悉吗？
    - 答c++一般般，java熟悉一点
- 问你个java的题，从10个数中（9个一样的），找出那个不同的数
    - 我当时脑子短路了，靠，我的想法是先取第一个数，分两种情况，可能是相同的，也可能是不同的
    进行一波遍历，如果第二个和第一个相同，那么继续遍历到不同的，那个就是我们要的，如果第一个不同的，继续遍历的话，肯定没有和不同的相同的了，那么说明第一个就是我们想要的。
- 函数指针和普通指针的区别
    - 真不知道，感觉是差不多多，问完珍惜表哥了，大概是函数指针需要从c++的面向对象的角度去考虑，两个结构体不一样，函数指针存的是类的偏移，也就是函数的入口地址。
- 有没分析过app，讲解一下
    - 一般分析都是从ctf中的安卓题，进行分析，更多偏逆算法
- 分析app的流程讲解下
    - 一般先从自己想入手的部分开始，比如想对某些接口做协议分析，从抓包开始，从字段信息以及栈打印信息中，定位到关键代码，看字段的加密是由哪些函数构成的，native函数就打开ida，分析，也可以使用frida-rpc的技术，直接调用，或者把算法逆向出来，本地复现。
- 剩下就是leader面了，问了一些生活方面的
    - 按自己的内心去回答了。  
- 总结: 
    - 感觉一些基础性问题没回答上，非常不应该，意识到自己的语言方面薄弱，c++尤其要加强，珍惜表哥也给我说c++非常重要，其次就是缺乏实际的app分析实战，这两点得下点功夫了，希望leader能给我一个机会，好运🍀。